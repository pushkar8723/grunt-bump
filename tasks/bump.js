...
module.exports = function(grunt) {
    ...
    grunt.registerTask('bump', DESC, function(versionType, incOrCommitOnly) {
        ...
        //branch check
        var branchAllowed = true;
        if (opts.gitAllowedBranches.length > 0) {
            var branchName = execSync('git branch | grep -e "^*"').toString().substr(2).trim();
            if (opts.gitAllowedBranches.indexOf(branchName) == -1) {
                grunt.log.writeln("Branch not configured to bump version.");
                branchAllowed = false;
            }
        }
